...
def news_tweet_agent():
    ...
    params = {
        ...
        "cmd": "Go to twitter (im already signed in). Search for the last tweets i made (check the last 1 tweets). Remember them so then you can go a search for information on Stanfords DSPy. Search for information on Stanfords DSPy on up to 3 different sources. If you see that the source has not really interesting information on Stanfords DSPy or i already made a tweet about that, then go to a different one. When you finish the research, go and make a few small and interesting DSPy tweets with the info you gathered. Make sure the tweet is small but informative and interesting for AI enthusiasts. Don't do more than 5 tweets",
        "maxSteps": 100,
    }
